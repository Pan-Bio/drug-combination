...
t=read.table('4-demo-real.txt',stringsAsFactor=F)
target=unique(t[,2])
drug=unique(t[,1])

write.csv(drug,"5-drug.csv")
...
alldrug=as.character(a[which(a[,2]>250),1])
#zhaochulex  indeyaowu
alldrug2 <- alldrug[!alldrug %in% ("none")]


alldrug=unique(c(alldrug2,drug))

#write.csv(alldrug,"alldrug.csv")

drug3 <- drug[drug %in% database]
alldrug3 <- alldrug[alldrug %in% database]

uni_drug<-setdiff(alldrug3, drug)

write.csv(uni_drug,"5-new_drug_new.csv")
alldrug<-alldrug3
...
write.table(file='5-sd_drugnet.txt',sd_drugnet,row.names = FALSE,col.names = FALSE,quote = FALSE)
write.table(file='5-sd_drugname.txt',alldrug,row.names = FALSE,col.names = FALSE,quote = FALSE)
...
write.table(file='5-st_drugnet.txt',st_drugnet,append=T,row.names = FALSE,col.names = FALSE,quote = FALSE)
write.table(file='5-st_drugname.txt',drug,append=T,row.names = FALSE,col.names = FALSE,quote = FALSE)
########################################################
...
dcate=as.matrix(unique(pt))
a=as.data.frame(table(pt))
catename=as.character(a[which(a[,2]>50),1])
...
write.table(file='5-sd_drugfeature46.txt',sd_drugfeature,row.names = FALSE,col.names = FALSE,quote = FALSE)
#write.table(file='5-sd_drug_name.txt',alldrug,row.names = FALSE,col.names = FALSE,quote = FALSE)
